...
class Crud_generator extends CI_Controller
{
    ...
    {
        parent::__construct();
        $this->load->helper(array("url", "crud", "form", "array"));
        $this->load->database();
    }
...
//        die();
    ...
    public function generar_entidad()
    {
        ...
        $saved_config = $this->_load_config($tabla);
        $columnas = $this->db->field_data($tabla);
        $tablas = $this->db->list_tables();
        $dataLayout = array();
        $dataPagina = array(
    ...
    public function generar()
    {
        if ($this->input->post("generar") !== FALSE) {
            ...
            $this->_guardar_config();
        }
    }

    private function _load_config($entidad)
    {
        $ruta_config_files = "./assets/crud_config_files/";
        $ruta_config_entidad = $ruta_config_files . $entidad;
        if (is_dir($ruta_config_entidad)) {
            $config_file = file_get_contents($ruta_config_entidad . "/{$entidad}.json");
            $config_decode = json_decode($config_file, TRUE);
            return $config_decode;
        }
        return NULL;
    }

    private function _guardar_config()
    {
        echo "<hr/><pre>" . __FILE__ . " L:" . __LINE__ . "<br>";
        ...
        $ruta_config_files = "./assets/crud_config_files/";
        $ruta_config_entidad = $ruta_config_files . $entidad;
        $fecha_config = date('Y-m-d_H-i');
        if ( ! is_dir($ruta_config_entidad)) {
            mkdir($ruta_config_entidad);
        }
        //file_put_contents($ruta_config_entidad . "/{$entidad}_{$fecha_config}.json", $postjson);
        file_put_contents($ruta_config_entidad . "/{$entidad}.json", $postjson);
        die();
    }
